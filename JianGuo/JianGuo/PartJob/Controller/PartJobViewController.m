...
{
    if (!_areaTypeArr) {
        _areaTypeArr = (NSMutableArray *)[CityModel city].list_t_area;
    }
    return _areaTypeArr;
}
...
-(void)requestList:(NSString *)count
{
    ...
    [JGHTTPClient getJobsListByHotType:self.type cityId:self.cityId areaId:self.areaId sequenceType:self.sequenceType count:count Success:^(id responseObject) {
        ...
        if (count.intValue) {//上拉加载            
            ...
            self.dataArr = [JianzhiModel mj_objectArrayWithKeyValuesArray:[responseObject[@"data"] objectForKey:@"list_t_job"]];
