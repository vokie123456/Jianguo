...
#import <objc/runtime.h> /** 给 textview 动态添加了一个label属性 来达到 placeholder 的效果 */
...
- (void)textDidChange:(NSNotification *)notify{
   self.label.hidden = self.text.length;//表示当label的字符长度为0时隐藏label,否则显示
}
...
- (UILabel *)label{
    ...
    if (label == nil) {
        ...
        label.textColor = RGBCOLOR(199, 199, 200);
        ...
        //关联到自身
        //参数 1.关联给谁 2.新添加的属性的名字 3.添加的属性的一个实例 4.关联到自身的设置参数
